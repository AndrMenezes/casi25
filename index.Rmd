...
--
- Reparameterise $\theta_j=\lambda_j/\sum_{k=1}^d \lambda_k$ and introduce the latent variiable $(\phi \mid \boldsymbol{\lambda}, \mathbf{y}) \sim \operatorname{Gamma}\lbrack N, \sum_{j=1}^d\lambda_j\rbrack$, such that the augmented joint p.m.f is

$$\Pr\lbrack\mathbf{Y}=\mathbf{y},\phi;\boldsymbol{\lambda}\rbrack=\dfrac{N!\phi^{N-1}}{\Gamma(N)}\prod_{j=1}^d\left\lbrack\dfrac{\lambda_j^{y_j}e^{-\lambda_j\phi}}{y_j!}\right\rbrack.$$

--
- We extend this to a zero-inflated Poisson form with a vector $\boldsymbol{\zeta} = (\zeta_1, \ldots, \zeta_d)$ of zero-inflation parameters for each category, such that $\zeta_j \in \lbrack 0, 1 \rbrack$, i.e.

$$\Pr\lbrack \mathbf{Y} = \mathbf{y}, \phi; \boldsymbol{\lambda}, \boldsymbol{\zeta} \rbrack =
\dfrac{N!\,\phi^{N - 1}}{\Gamma(N)}\,\prod_{j=1}^d \left\lbrack
\zeta_j\,I_0(y_{j}) + (1 - \zeta_j)\,\dfrac{\lambda_j^{y_{j}}\,e^{-\lambda_j\,\phi}}{y_{j}!}
\right\rbrack$$

and then integrate out $\phi$ to obtain the zero-and-N-inflated multinomial (ZANIM) distribution.
...
\right\rbrack \\
\fbox{Reduced multinomials}\rightarrow 
&\phantom{=}~+
...
\right\rbrack  
\quad \mathrm{for} \quad \mathbf{y} \in \pmb{\Omega}_{d,N} \cup \mathbf{0}_d,\end{align}$$
where
...
--
> We show that ZANIDM also admits a finite mixture representation and we derive moments, marginals, and fully conjugate Bayesian inference schemes for both distributions.
